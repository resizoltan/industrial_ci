...
if [ "$BUILDER" == catkin ]; then build-wrapper --out-dir "/root/sonar/bw_output" catkin build $OPT_VI --summarize  --no-status "${pkgs_whitelist[@]}" "${catkin_parallel_jobs[@]}" --make-args "${ros_parallel_jobs[@]}" ; fi
